...
- That EDC Connectors can interact to populate a federated catalog, and copying a file from a provider (`service`) to a
  consumer (`versicherung`) blob storage account.
...
e.g. `system-tests/resources/vault/service`. These were generated using the following commands:
...
Generated keys are imported to keystores e.g. `system-tests/resources/vault/service/service-keystore.jks`. Each
...
generated by converting the corresponding public key to JWK format, for example service connector public key was
converted to JWK using following command:

```bash
docker run -i danedmunds/pem-to-jwk:1.2.1 --public --pretty < system-tests/resources/vault/service/public-key.pem > key.public.jwk
```
...
Follow the instructions in the previous sections to run an MVD with a consumer (`versicherung`) and provider (`service`)
locally using `docker compose`.

Once running, you can use a Java debugger to connect to the consumer (`versicherung`, port 5006) and provider (`service`,
port 5005) instances. If you are using IntelliJ you can use the provided "EDC service", "EDC versicherung" or "EDC
