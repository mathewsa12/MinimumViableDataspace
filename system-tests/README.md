...
- That EDC Connectors can interact to populate a federated catalog, and copying a file from a provider (`controltower`) to a
...
e.g. `system-tests/resources/vault/controltower`. These were generated using the following commands:
...
Generated keys are imported to keystores e.g. `system-tests/resources/vault/controltower/controltower-keystore.jks`. Each
...
generated by converting the corresponding public key to JWK format, for example controltower connector public key was
...
Follow the instructions in the previous sections to run an MVD with a consumer (`bridge`) and provider (`controltower`)
locally using `docker compose`.

Once running, you can use a Java debugger to connect to the consumer (`bridge`, port 5006) and provider (`controltower`,
port 5005) instances. If you are using IntelliJ you can use the provided "EDC controltower", "EDC bridge" or "EDC
